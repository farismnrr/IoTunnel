...
class ProjectRepository {
    ...
    async addProject(id: string, userId: string, project: IProject): Promise<void> {
        ...
        const projectQuery = {
            text: `
                INSERT INTO projects (id, name, description, device_type, user_id, created_at, updated_at) 
                VALUES ($1, $2, $3, $4, $5, $6, $6) 
            `,
            values: [id, project.name, project.description, project.deviceType, userId, createdAt]
        };
    ...
    async getProjectById(id: string): Promise<IProject | null> {
        const projectQuery = {
            text: "SELECT id, name, description, device_type, device_ip_address, user_id FROM projects WHERE id = $1",
            values: [id]
        };
    ...
    async getProjectsByUserId(userId: string): Promise<IProject[]> {
        const projectQuery = {
            text: "SELECT id, name, description, device_type, device_ip_address, user_id FROM projects WHERE user_id = $1",
            values: [userId]
        };
        const projectResult = await this._pool.query(projectQuery);
    ...
    async getProjectByName(name: string, userId: string): Promise<IProject | null> {
        const projectQuery = {
            text: "SELECT id, name, description, device_type, device_ip_address, user_id FROM projects WHERE name = $1 AND user_id = $2",
            values: [name, userId]
        };
        const projectResult = await this._pool.query(projectQuery);
        return projectResult.rows[0] || null;
    ...
    async updateProject(id: string, project: IProject): Promise<void> {
        ...
        if (project.deviceType) {
            ...
            values.push(project.deviceType);
        }
    ...
    async updateProjectIpAddress(id: string, project: IProject): Promise<void> {
        const updatedAt = new Date();
        const fields: string[] = [];
        const values: (string | number)[] = [];
        let index = 1;

        if (project.deviceIpAddress) {
            fields.push(`device_ip_address = $${index++}`);
            values.push(project.deviceIpAddress);
        }
